...
function Create() {
  ...
  useEffect(() => {
    ...
    const CREATE_URL = "http://localhost:3000/api/home"
    const POST_URL = "http://localhost:3000/api/post/upload";

    const token = localStorage.getItem("authToken");
    axios.defaults.headers.common["Authorization"] = `Bearer ${token}`;

    if(token){
      axios.get(CREATE_URL).then((response: { data: { email: any; username: any; }; }) => {
        if (
          response &&
          response.data &&
          response.data.email &&
