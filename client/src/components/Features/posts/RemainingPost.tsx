...
import { updateLikeUrl } from "../../../api/axiosConnection";
...
function RemainingPosts({ card }: { card: card }) {
    ...
    const [isLike, setIsLike] = useState<boolean>(card?.likes?.some((user:string)=>user===username));
  ...
  return (
    ...
    {isLikeList?<LikesModal likes={card?.likes} setIsLikeList={setIsLikeList} />:null}
    {isFullImage?<FullImageModal url={backgroundImage} setIsFullImage={setIsFullImage} image={card.image}/>:null}
