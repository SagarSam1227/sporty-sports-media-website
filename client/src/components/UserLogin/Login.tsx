...
import { LoginProps, userResponse } from '../../vite-env';
...
function Login({ open, setOpen }: LoginProps) {
    ...
    useEffect(() => {
        if (isLoginClicked) {
            ...
            const API_URL = 'http://localhost:3000/api/user/get-by-email'
            ...
            axios.post(API_URL,data).then((response:userResponse)=>{
                console.log(response);
                if(response){
                   console.log(response.data);
                   handleItem(response.data?.email,response.data?.username) 
                }
                else{
